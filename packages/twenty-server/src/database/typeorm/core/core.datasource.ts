...
const runtimeSourceDirectory = isJest ? 'src/' : 'dist/src/';
...
export const typeORMCoreModuleOptions: TypeOrmModuleOptions = {
  ...
  entities:
    process.env.IS_BILLING_ENABLED === 'true'
      ? [
          `${runtimeSourceDirectory}engine/core-modules/**/*.entity{.ts,.js}`,
          `${runtimeSourceDirectory}engine/metadata-modules/**/*.entity{.ts,.js}`,
        ]
      : [
          `${runtimeSourceDirectory}engine/core-modules/**/!(billing-*).entity.{ts,js}`,
          `${runtimeSourceDirectory}engine/metadata-modules/**/*.entity{.ts,.js}`,
        ],
  synchronize: false,
  migrationsRun: false,
  ...
  migrations:
    process.env.IS_BILLING_ENABLED === 'true'
      ? [
          `${runtimeSourceDirectory}database/typeorm/core/legacy-typeorm-migrations-do-not-add/common/*{.ts,.js}`,
          `${runtimeSourceDirectory}database/typeorm/core/legacy-typeorm-migrations-do-not-add/billing/*{.ts,.js}`,
        ]
      : [
          `${runtimeSourceDirectory}database/typeorm/core/legacy-typeorm-migrations-do-not-add/common/*{.ts,.js}`,
        ],
